...
export const VotingMethodIcon = ({ votingMethod }: VotingMethodIconProps) => {
  ...
  return (
    <details className="group relative shrink-0">
      <summary aria-label={label} className="list-none rounded-app p-1 text-app-primary hover:bg-app-info [&::-webkit-details-marker]:hidden">
      ...
      <span className="absolute top-full right-0 z-10 mt-1 hidden w-max max-w-40 rounded-app bg-app-text px-2 py-1 text-center font-semibold text-app-inverse shadow-lg group-open:block md:group-hover:block md:group-open:hidden">
        {label}
      </span>
    </details>
  );
