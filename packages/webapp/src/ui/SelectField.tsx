...
export const SelectField = ({ className = "", label, name, onChange, options, value }: SelectFieldProps) => (
  <label className={cx("block font-semibold", className)}>
    {label}
    <select className={`mt-1.5 w-full rounded-app border border-app-border bg-app-surface px-3 py-2.5 font-normal ${className}`} name={name} onChange={onChange} value={value}>
      {options.map((option) => (
